...
Touch ID side does not depend on the renderer. Log out and back in, or let the omarchy-t1
installer start a renderer for this session.
...
Restart the broker and retry once; omarchy-t1 does that by itself. If it fails again, check
`sudo t1bridge status` for `xart: ready` and the firewall rule below.
...
not that the T1 can reach it. omarchy-t1 adds the scoped rule.
