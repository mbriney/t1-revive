...
| `esp` | one block per EFI System Partition: device path, whether it is on a removable disk (a USB stick's own EFI partition is a candidate the selection rule skips), mounted, `EFI/APPLE` present, `EMBEDDEDOS` present, and for `combined.memboot`, `FDRData`, `version.plist` a yes/no plus size rounded to KB. A partition that is not mounted (Apple's ESP on a dual-boot Mac) is looked at through a read-only probe mount when the report runs as root (`note: probed-read-only`); otherwise its fields stay `?` (`note: not-mounted-needs-root`). Then `esp-candidates`, `esp-selected` (the device the tool would use, or `ambiguous`) and `esp-selected-why`. Names and sizes only; nothing is opened |
