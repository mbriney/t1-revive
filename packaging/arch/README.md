...
`VERSION` is `0.1.0-dev`; a `pkgver` may not contain `-`, so the PKGBUILD carries both:

```
_relver=0.1.0-dev     # the git tag is v$_relver, and VERSION must equal this
pkgver=0.1.0.dev      # the Arch version
```
...
mkdir -p "$work/src/t1-revive-0.1.0-dev"
git -C ../.. archive --format=tar HEAD | tar -C "$work/src/t1-revive-0.1.0-dev" -xf -
for t in "$work"/dl/*.tar.gz; do tar -C "$work/src" -xzf "$t"; done   # -> <name>-<commit>/
# rename each <name>-<commit> to vendor/src/<name>, then:
( cd "$work/src/t1-revive-0.1.0-dev" && bash build.sh --offline --prefix "$work/src/prefix" )
```
